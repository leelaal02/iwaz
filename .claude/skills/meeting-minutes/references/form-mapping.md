...
- `todo` — **양식에만 있고 대응 데이터가 없는 값 칸/컬럼**(작성자·회의장소·장소·미결사항·다음회의·첨부 등)에 빨간 굵은 "입력필요"를 넣는다. `fields` 불필요. **회의 데이터(minutes.json)에 없는 값 슬롯에만** 쓴다(값이 있으면 `inline`/`block`으로 채움).
...
- **`todo`를 언제 쓰는지 헷갈리면 이 기준으로**: 9항목에 **값이 있으면 절대 `todo`로 덮지 않는다.** `inline`/`block`은 스크립트가 값 유무를 보고 자동 처리하므로(값이 있으면 값, 없으면 "입력필요") 그냥 매핑하면 된다. `todo`는 9항목에 대응 자체가 없는 양식 전용 칸에만 쓴다.
